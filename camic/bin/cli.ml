...
let parse_command_line () =
  ...
  let speclist =
    [
      ...
      ("-show_ast", Arg.Set is_show_ast, "Output the product of parser, which is AST");
      ("-source_charset", Arg.Set_string Config.source_charset, "Set the encoding of source file, UTF-8 by default");
      ("-execution_charset", Arg.Set_string Config.execution_charset, "Set the encoding of execution environment for multi-byte character, UTF-8 by default");
      ("-execution_wide_charset", Arg.Set_string Config.execution_wide_charset, "Set the encoding of execution environment for wide character, UTF-16(if wide_size == 2) or UTF-32(if wide_size == 4) by default");
