...
CAMI 接收文本或二进制形式（暂未支持）的字节码文件（参见[CAMI字节码规范](./cami_bytecode.md)）作为蓝本进行执行。若传入的字节码文件是未经链接，CAMI会自动（递归地）寻找该文件依赖的其他字节码文件并链接。
...
然而，鱼和熊掌不可兼得，CAMI 自然也存在其缺陷，最主要的问题在于其执行效率。针对计算密集型程序，CAMI 的执行效率比同等逻辑的 CPython 程序低大约1至1.5个数量级。为解决该缺陷，我们预计采用 JIT 技术或硬件加速技术。
...
同时，由于 C 标准对于 allocated 存储周期对象的 effective type 的规定和其他语义规定（如指针加法）存在一定不自洽性，我们对 malloc 的行为进行了修改，修改后的行为和 C++/Java 的 new 运算符类似。
...
### 检测能力评估
为了对比不同检测软件的检测能力，我们构建了178个测试用例并分为阳性和隐性两个测试套件。阳性套件用以检测检测率，隐性套件用以检测假阳性率。更多细节参见[评估指南](./evaluation.md)。如下为评估结果：
...
## 构建
注意：
+ 后文将用 `${PROJECT_ROOT}` 指代本项目根目录
+ 本项目现仅可构建在在类 Unix 平台下
### 配置（可选的）
...
├── evaluation                    # 项目评估目录
│   ├── bin
│   │   └── cami.sh              # 处理命令行参数，将C语言源文件改为预编译好的字节码文件传递给 CAMI
│   ├── config
│   │   └── config.json          # 配置参与评估的软件信息
│   ├── Dockerfile                # 构建评估环境的 Docker 构建脚本
│   ├── out
│   │   └── CAMI                 # 预编译的测试文件
│   ├── requirements.txt          # 自动化评估脚本依赖包
│   ├── script                    # 自动化评估脚本
│   └── test_suite                # 测试用例
├── include                        # CAMI 头文件
├── LICENSE                        # GPLv2 许可证
